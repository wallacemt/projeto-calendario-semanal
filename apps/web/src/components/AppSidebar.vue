...
<template>
  <div class="relative  glass flex flex-shrink-0 flex-col gap-1 overflow-hidden py-7 px-4 " :style="{
    width: `${width}px`,
    transition: drag.active ? 'none' : 'width 0.28s cubic-bezier(.4,0,.2,1)',
    background:
      'radial-gradient(ellipse 260px 220px at 15% 0%, rgba(139,92,246,0.24), transparent 60%), radial-gradient(ellipse 240px 280px at 100% 90%, rgba(56,189,248,0.2), transparent 65%), rgba(0,0,0,0.6)',
    ...
    <RouterLink :to="{ name: 'profile' }"
      ...
      <div v-else class="flex h-8.5 w-8.5 flex-shrink-0 items-center justify-center rounded-full text-[13px] font-bold"
