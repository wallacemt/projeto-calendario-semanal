const animesSegunda = {
  conteudo1: {
    tituloBanner: "Jidou Hanbaiki 2nd Season",
    imagemBanner:
      "https://img.animeschedule.net/production/assets/public/img/anime/jpg/default/jidou-hanbaiki-ni-umarekawatta-ore-wa-meikyuu-wo-samayou-2nd-season-3d00725d44.jpg",
    subtituloBanner: "Estúdio: AXsiZ, Studio Gokumi \n\nGêneros: Comedy Fantasy Isekai Reincarnation",
    linkBanner: "https://smartanimes.com/anime/jidou-hanbaiki-ni-umarekawatta-ore-wa-meikyuu-wo-samayou-2nd-season",
  },
  conteudo2: {
    tituloBanner: "Grand Blue Season 2",
    imagemBanner:
      "https://a.storyblok.com/f/178900/1064x1504/562ae656c6/grand-blue-dreaming-season-2-teaser-visual.jpg",
    subtituloBanner: "Estúdio:  Studio A-CAT \n\nGêneros: Fantasy, Reincarnation",
    linkBanner: "https://smartanimes.com/anime/grand-blue-season-2",
  },
};

const animesTerca = {
  conteudo1: {
    tituloBanner: "Sono Bisque Doll 2nd",
    imagemBanner:
      "https://external-preview.redd.it/my-dress-up-darling-season-2-key-visual-2025-v0-WhwVaS8qKLkeqHNaL_CayjVzLYODCY_rNl42pTt4v8k.jpg?auto=webp&s=1d1e11c2a65fd49ee6d09805787a906c39b05392",
    subtituloBanner: "Estúdio: Elias\n\nGêneros: Comédia, Fantasia",
    linkBanner: "https://smartanimes.com/anime/sono-bisque-doll-wa-koi-wo-suru-season-2",
  },
  conteudo2: {
    tituloBanner: "Dekichau Made Kon",
    imagemBanner: "https://nami.news/wordpress/wp-content/uploads/2025/05/Dekichau-Made-Kon-Illustration.jpeg",
    subtituloBanner: "Estúdio:  Felix Film \n\nGêneros: Comedy, Love Status Quo, Romance, School, Shounen",
    linkBanner: "https://www.muitohentai.com/info/dekichau-made-kon/",
  },
};

const animesQuarta = {
  conteudo1: {
    tituloBanner: "Tate no Yuusha no Nariagari 4nd",
    imagemBanner:
      "https://a.storyblok.com/f/178900/1000x1419/db446357ef/the-rising-of-the-shield-hero-season-4-visual.jpg",
    subtituloBanner: "Estúdio: Satelight, Staple Entertainment\n\nGêneros: Action, Delinquents, School",
    linkBanner: "https://smartanimes.com/anime/tate-no-yuusha-no-nariagari-season-4",
  },
  conteudo2: {
    tituloBanner: "Clevatess: Majuu no Ou to Akago to Shikabane no Yuusha",
    imagemBanner: "https://i0.wp.com/anitrendz.net/news/wp-content/uploads/2024/07/clevatess_teaservisual.jpg?ssl=1",
    subtituloBanner: "Estúdio: Bandai Namco Pictures\n\nGêneros: Comedy Music School Seinen",
    linkBanner: "https://smartanimes.com/anime/clevatess-majuu-no-ou-to-akago-to-shikabane-no-yuusha",
  },
};

const animesQuinta = {
  conteudo1: {
    tituloBanner: "Jigoku Sensei Nube",
    imagemBanner:
      "https://m.media-amazon.com/images/M/MV5BMWQ1MTc5ZDYtODVlMy00OWZjLWFiYjAtNDQ4MTc3ODRiNmE1XkEyXkFqcGc@._V1_.jpg",
    subtituloBanner: "Estúdio: Yokohama Animation Laboratory\n\nGêneros:Action, Adventure, Fantasy, Historical",
    linkBanner: "https://nube-anime.com/",
  },
  conteudo2: {
    tituloBanner: "Tougen Anki",
    imagemBanner: "https://a.storyblok.com/f/178900/750x1061/966c93f71d/tougen_anki_key_art.jpg",
    subtituloBanner: "Estúdio: MAPPA\n\nGêneros: Sci-Fi",
    linkBanner: "https://smartanimes.com/anime/tougen-anki",
  },
};

const animesSexta = {
  conteudo1: {
    tituloBanner: "Sakamoto Days 2nd",
    imagemBanner: "https://uploads.jovemnerd.com.br/wp-content/uploads/2025/03/gme4g7tasaalk9q__r2h1p9-760x950.jpg",
    subtituloBanner: "Estúdio: TMS Entertainment\n\nGêneros:Action, Adventure, Fantasy",
    linkBanner: "https://smartanimes.com/anime/sakamoto-days-part-2",
  },
  conteudo2: {
    tituloBanner: "Gachiakuta",
    imagemBanner: "https://m.media-amazon.com/images/I/61u3ItUwldL._UF1000,1000_QL80_.jpg",
    subtituloBanner: "Estúdio:  LAN Studio, Paper Plane Animation Studio\n\nGêneros:Action Super Power",
    linkBanner: "https://smartanimes.com/anime/gachiakuta",
  },
};
const animesSabado = {
  conteudo1: {
    tituloBanner: "Dandadan 2nd",
    imagemBanner:
      "https://external-preview.redd.it/dandadan-season-2-new-visual-v0-l9HdtMCFBMH-DZg3T7r1C00BWFeXPhRuGwYig2EQXOs.jpg?auto=webp&s=851cfbb8101faeadcfc3ffb6cb917505b9e9a0c2",
    subtituloBanner: "Estúdio: David Production\n\nGêneros: Action, Fantasy, Sci-Fi, Shounen, Urban Fantasy",
    linkBanner: "https://smartanimes.com/anime/dandadan-2nd-season",
  },
  conteudo2: {
    tituloBanner: "Kaijuu 8-gou 2nd",
    imagemBanner: "https://nami.news/wordpress/wp-content/uploads/2024/12/Kaijuu-8-gou-Anime-sr-Season-Visual.jpg",
    subtituloBanner: "Estúdio: TMS Entertainment\n\nGêneros: Fantasia",
    linkBanner: "https://smartanimes.com/anime/kaijuu-8-gou-2nd-season",
  },
};
const animesDomingo = {
  conteudo1: {
    tituloBanner: "One Piece🏴‍☠️",
    imagemBanner: "https://assets.gamearena.gg/wp-content/uploads/2023/07/29231448/One-Piece-Vol-106.jpg.webp",
    subtituloBanner:
      "Autor: Eiichiro Oda\n\nDireção: Konosuke Uda\n\nEstúdio: Toei Animation\n\nStatus: Infinito\n\nGêneros: Ação Aventura Comédia Drama Fantasia Shounen Superpoder",
    linkBanner: "https://smartanimes.com/anime/one-piece",
  },
  conteudo2: {
    tituloBanner: "Dr Stone 2nd",
    imagemBanner: "https://pbs.twimg.com/media/GCH6xkjWoAAvXzg.jpg:large",
    subtituloBanner:
      "Autor: Eiichiro Oda\n\nDireção: Konosuke Uda\n\nEstúdio: Toei Animation\n\nStatus: Infinito\n\nGêneros: Ação Aventura Comédia Drama Fantasia Shounen Superpoder",
    linkBanner: "https://smartanimes.com/anime/dr-stone-science-future-part-2",
  },
};
const animesExtra = [
  {
    conteudo1: {
      tituloBanner: "Vigilante: Boku no Hero Academia Illegals",
      imagemBanner: "https://gqcanimes.com.br/wp-content/uploads/2025/01/My-Hero-Academia-Vigilantes-anime.webp",
      subtituloBanner: "Gênero: Action, Shounen, Super Power",
      linkBanner: "https://smartanimes.com/anime/vigilante-boku-no-hero-academia-illegals",
    },
  },
];
